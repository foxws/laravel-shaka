...
it('moves files onto a local disk instead of copying them', function () {
    ...
    (new PackagerResult('success', null, $this->source))->toDisk($this->target, null, true, 'playlist/');
...
it('applies visibility to moved files', function () {
    (new PackagerResult('success', null, $this->source))->toDisk($this->target, 'private');
...
it('copies instead of moving when temporary files are kept', function () {
    (new PackagerResult('success', null, $this->source))->toDisk($this->target, null, false);
