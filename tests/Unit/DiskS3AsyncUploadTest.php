 ...
 * exercise PackagerResult's async S3 upload path (isS3Disk() is false for a
...
it('uploads files concurrently to an S3-compatible disk via async promises', function () {
    ...
    $result = new PackagerResult('success', null, $tempDir, null, ['concurrency_workers' => 5]);
...
it('uploads files above the multipart threshold as a multipart upload', function () {
    ...
    $result = new PackagerResult('success', null, $directory, null, [
...
it('aborts a failed multipart upload and reports the failure', function () {
    ...
    $result = new PackagerResult('success', null, $directory, null, ['multipart_threshold' => 1024 * 1024]);
...
it('uploads encryption keys as octet-stream instead of a keynote document', function () {
    ...
    (new PackagerResult('success', null, $directory))->toDisk($filesystem);
